...
class Mouse2(Turtle):
    ...
    def getnextstate(self,reset=0):
        ...
        relativeVector=self.position-Turtle.center
        ...
        #    self.mouseAngle -= 360.
        Turtle.mouseAngle-=9/(5*pi)
        # if reset, return to original position
        if reset:
            return self.originalPosition, Turtle.mouseAngle-90
        else:
            # this allows mouse to circle the statue by changing the mouseAngle
            return Turtle.center + 100.*unit(Turtle.mouseAngle),Turtle.mouseAngle-90
